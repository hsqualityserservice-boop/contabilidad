...
export default function AuthForm({ mode = 'sign-in' }: { mode?: 'sign-in' | 'sign-up' }) {
  ...
  return <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-900/5 sm:p-10">
    ...
    <div className="mb-7"><h1 className="text-2xl font-bold tracking-tight text-[#152238]">{mode === 'sign-up' ? 'Crea tu cuenta' : 'Bienvenido de nuevo'}</h1><p className="mt-2 text-sm text-slate-500">{mode === 'sign-up' ? 'Crea tu acceso al espacio de H&amp;S Quality Service.' : 'Accede a tu espace de gestion H&amp;S Quality Service.'}</p></div>
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      {mode === 'sign-in' && <div className="-mt-2 text-right"><a href="mailto:soporte@hs-cleaning.ch?subject=Récupération%20de%20mot%20de%20passe" className="text-xs font-semibold text-[#c1121f] hover:underline">Mot de passe oublié ?</a></div>}
    ...
    <p className="mt-8 text-center text-xs text-slate-400">Acceso seguro para los espacios H&amp;S</p>
    <p className="mt-4 text-center text-sm text-slate-500">{mode === 'sign-up' ? '¿Ya tienes una cuenta?' : '¿Aún no tienes una cuenta?'} <Link href={mode === 'sign-up' ? '/sign-in' : '/sign-up'} className="font-semibold text-[#c1121f] hover:underline">{mode === 'sign-up' ? 'Inicia sesión' : 'Crear cuenta'}</Link></p>
  </div>
}
