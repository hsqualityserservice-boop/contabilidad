...
export default function AuthForm({ mode = 'sign-in' }: { mode?: 'sign-in' | 'sign-up' }) {
  ...
  return <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-900/5 sm:p-10">
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <button disabled={pending} className="mt-2 flex h-11 items-center justify-center gap-2 rounded-xl bg-[#ea580c] text-sm font-semibold text-white transition hover:bg-[#c2410c] disabled:cursor-not-allowed disabled:opacity-60">{pending ? (mode === 'sign-up' ? 'Creando cuenta...' : 'Accediendo...') : (mode === 'sign-up' ? 'Crear cuenta' : 'Iniciar sesión')}{!pending && <ArrowRight className="size-4" />}</button>
    </form>
    <p className="mt-8 text-center text-xs text-slate-400">Acceso seguro para administradores de NEXUS.OS</p>
    <p className="mt-4 text-center text-sm text-slate-500">{mode === 'sign-up' ? '¿Ya tienes una cuenta?' : '¿Aún no tienes una cuenta?'} <Link href={mode === 'sign-up' ? '/sign-in' : '/sign-up'} className="font-semibold text-[#ea580c] hover:underline">{mode === 'sign-up' ? 'Inicia sesión' : 'Crear cuenta'}</Link></p>
  </div>
}
