...
export default function AuthForm({ mode = 'sign-in' }: { mode?: 'sign-in' | 'sign-up' }) {
  ...
  return <div className="relative w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 shadow-xl shadow-slate-900/5 sm:p-10">
    <div className="absolute right-6 top-6 flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 p-1" aria-label="Language selector">
      {(['FR', 'ES', 'EN'] as Locale[]).map((item) => <button key={item} type="button" onClick={() => setLocale(item)} aria-pressed={locale === item} className={`rounded-full px-2 py-1 text-[10px] font-bold ${locale === item ? 'bg-[#c1121f] text-white' : 'text-slate-500'}`}>{item}</button>)}
    </div>
    ...
    <div className="mb-7"><h1 className="text-2xl font-bold tracking-tight text-[#152238]">{isSignIn ? 'Bienvenue' : t.title}</h1><p className="mt-2 text-sm text-slate-500">{isSignIn ? 'Accédez à votre espace de gestion H&S Quality Service.' : t.subtitle}</p></div>
