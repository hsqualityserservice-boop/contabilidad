...
export function ClientWorkspace({ lang, t, onNotice }: { lang: Lang; t: ClientCopy; onNotice: (message: string) => void }) {
  ...
  async function startPayment() {
    ...
    try {
      const url = await createPaymentSession('cleaning-intervention')
      if (url) window.location.assign(url)
    } catch {
      onNotice('Payment unavailable. Please try again.')
    } finally {
